...
/**
 * This utility class is used to store content in Android's Shared Preferences. For maximum security the preferences should be private.
 */
public class AmazonSharedPreferencesWrapper {
	...
	/**
	 * Set all of the Shared Preferences used by the Token Vending Machine to null. This function is useful if the user needs/wants to log out to
	 * clear any user specific information.
	 */
	...
	/**
     * Stores the username that was registered in the Shared Preferences.
     */       
    public static void storeUsername( SharedPreferences sharedPreferences, String username ) {
        AmazonSharedPreferencesWrapper.storeValueInSharedPreferences( sharedPreferences, AWS_DEVICE_USERNAME, username );
    }
	...
	/**
	 * Stores the UID and Key that were registered in the Shared Preferences. The UID and Key and used to encrypt/decrypt the Token that is returned
	 * from the Token Vending Machine.
	 */
	...
	/**
	 * Checks if the current Token's expiration date stored in Shared Preferences has expired. A token that has expired or will expire in fifteen
	 * minutes or less is considered expired.
	 */
